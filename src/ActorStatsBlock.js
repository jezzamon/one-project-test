...
const StatsTitle = styled.h2`
  ...
  ${({ configs: { statTitleColor } }) =>
    statTitleColor && `color:${statTitleColor}`}
`;
...
const StatsValue = styled.span`
  ${({ configs: { statValueColor } }) =>
    statValueColor && `color:${statValueColor}`}
`;
...
const ActorStatsBlock = ({
  ...
  return (
    <Wrapper configs={configs}>
      ...
      {isMobile ? (
        <CollapseBody
          {...(hasExpandCollapse ? { isOpen: isExpanded } : { isOpen: true })}
        >
          <StatsList>
...
export default ActorStatsBlock;
